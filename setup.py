...
setup(
    name="ivadomed-" + ("anima-bin" if not ANIMA_APP else ANIMA_APP.removesuffix(".exe")),
    description="ANIMA medical image processing program" + ("s" if not ANIMA_APP else ": " + ANIMA_APP.removesuffix(".exe")),
    ...
    install_requires = (
        [f'ivadomed-{bin.removesuffix(".exe")}=={setuptools_scm.get_version()}' for bin in ANIMA_APPS]
        if not ANIMA_APP else
        []
        ),
